...
-- Tick Mario in sync with the GMod game loop using a delta-time accumulator.
-- This avoids the drift/misfire issues of an independent OS timer.
G64_TICKRATE = 1/33
local g64TickAccum = 0

hook.Add("Think", "G64_GAME_TICK", function()
    local dt = FrameTime()
    if dt <= 0 then return end

    g64TickAccum = g64TickAccum + dt

    -- Drain the accumulator in fixed steps, capped to avoid spiral-of-death
    -- if the game hitches.
    local maxTicks = 4
    while g64TickAccum >= G64_TICKRATE and maxTicks > 0 do
        hook.Call("G64GameTick")
        g64TickAccum = g64TickAccum - G64_TICKRATE
        maxTicks = maxTicks - 1
    end

    -- Clamp residual so a long hitch doesn't cause a burst of ticks on recovery.
    if g64TickAccum > G64_TICKRATE then
        g64TickAccum = G64_TICKRATE
    end
end)
...
hook.Add("G64AdjustedTimeScale", "G64_ADJUST_TIMESCALE", function(timeScale)
    ...
    g64TickAccum = 0  -- Reset accumulator so old debt doesn't carry over.
end)
