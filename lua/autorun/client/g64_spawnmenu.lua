...
hook.Add("PopulateToolMenu", "G64_CREATE_MENU_SETTINGS", function()
	...
	spawnmenu.AddToolMenuOption("Utilities", "G64", "G64_Settings", "#Settings", "", "", function(panel)
		...
		local movementOnly = vgui.Create("DCheckBoxLabel")
		movementOnly:SetText("Movement Only mode")
		movementOnly:SetTextColor(Color(0,0,0))
		movementOnly:SetTooltip([[When enabled, Mario only handles movement.
		Your player model stays visible, Mario can't
		take damage, and you keep your normal health/weapons.
		Toggle this BEFORE spawning Mario.]])
		if GetConVar("g64_movement_only"):GetBool() then movementOnly:SetValue(true)
		else movementOnly:SetValue(false) end
		movementOnly:SetConVar("g64_movement_only")
		...
		panel:AddItem(movementOnly)

		local vrAttackMode = vgui.Create("DNumSlider")
		vrAttackMode:SetText("VR Attack Mode (0=trigger, 1=motion, 2=both)")
		vrAttackMode:SetMin(0)
		vrAttackMode:SetMax(2)
		vrAttackMode:SetDecimals(0)
		vrAttackMode:SetConVar("g64_vr_attack_mode")
		vrAttackMode:SetDark(true)

		local vrPunchThresh = vgui.Create("DNumSlider")
		vrPunchThresh:SetText("VR Punch Velocity Threshold")
		vrPunchThresh:SetMin(0.1)
		vrPunchThresh:SetMax(10)
		vrPunchThresh:SetDecimals(1)
		vrPunchThresh:SetConVar("g64_vr_punch_threshold")
		vrPunchThresh:SetDark(true)

		local vrCrouchH = vgui.Create("DNumSlider")
		vrCrouchH:SetText("VR Crouch Detection Height")
		vrCrouchH:SetMin(10)
		vrCrouchH:SetMax(80)
		vrCrouchH:SetDecimals(0)
		vrCrouchH:SetConVar("g64_vr_crouch_height")
		vrCrouchH:SetDark(true)

		panel:AddItem(vrAttackMode)
		panel:AddItem(vrPunchThresh)
		panel:AddItem(vrCrouchH)

		local vrDebugCrouch = vgui.Create("DCheckBoxLabel")
		vrDebugCrouch:SetText("VR Crouch Debug HUD")
		vrDebugCrouch:SetTextColor(Color(0,0,0))
		vrDebugCrouch:SetConVar("g64_vr_debug_crouch")

		panel:AddItem(vrDebugCrouch)
	...
	spawnmenu.AddToolMenuOption("Utilities", "G64", "G64_KeyBinds", "#Key Binds", "", "", function(panel)
		...
		local header_pickup = vgui.Create("DLabel")
		header_pickup:SetText("Pick Up")
		header_pickup:SetTextColor(Color(0,0,0))
		local binder_pickup = vgui.Create("DBinder")
		binder_pickup:SetConVar("g64_pickup")
		...
		panel:AddItem(header_pickup)
		panel:AddItem(binder_pickup)
