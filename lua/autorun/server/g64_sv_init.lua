...
local health = net.ReadUInt(4)
		if not GetConVar("g64_movement_only"):GetBool() then
...
hook.Add("EntityTakeDamage", "G64_PLAYER_DAMAGED", function(target, dmg)
	if target:IsPlayer() and target.IsMario == true and target:HasGodMode() == false then
		-- In movement-only mode, don't intercept damage — let normal GMod damage handling occur
		if GetConVar("g64_movement_only"):GetBool() then
			return
		end
...
hook.Add("PlayerDeath", "G64_PLAYER_DEATH", function(victim, inflictor, attacker)
	if IsValid(victim.MarioEnt) then
		-- In movement-only mode, don't destroy Mario on player death
		-- Mario persists and player will respawn at Mario's position
		if not GetConVar("g64_movement_only"):GetBool() then
