...
function ENT:Initialize()
	...
	self.MovementOnly = GetConVar("g64_movement_only"):GetBool()
	self.vrSwimming = false
	self.vrCrouchActive = false
	...
	net.Receive("G64_PLAYERREADY", function(len, ply)
		...
		if ply == self.Owner then
			if not self.MovementOnly then
				...
				-- Suppress player movement (skip if VRMod active or swimming)
				hook.Add("SetupMove", "G64_MOVEMENT_ONLY_SETUPMOVE" .. self.MarioId, function(moveply, mv)
					if moveply == self.Owner and IsValid(self) then
						if g_VR and g_VR.active then return end
						if self.vrSwimming then return end
	...
	if self.Invalid == false then
		if CLIENT then
			if libsm64 ~= nil and libsm64.ModuleLoaded then
				...
				hook.Remove("CalcViewModelView", "G64_CALCVIEWMODELVIEW" .. self.MarioId)
				...
				hook.Remove("HUDPaint", "G64_VR_DEBUG_CROUCH" .. self.MarioId)
			...
			hook.Remove("SetupMove", "G64_MOVEMENT_ONLY_SETUPMOVE" .. self.MarioId)
...
if CLIENT then
	...
	function ENT:Draw()
		if self.MovementOnly and self.Owner == LocalPlayer() then return end
	...
	function ENT:StartLocalMario()
		...
		-- NEW: Variables to track smooth time and subtract paused time
		self.pauseOffset = 0
		self.lastSysTime = SysTime()

		-- SetupMove hook is added after MarioId is created so there is only one active hook.
		...
		if self.MarioId == nil or self.MarioId < 0 then
			...
			self:SetNoDraw(false)
hook.Add("SetupMove", "G64_CLIENT_MOVEMENT_" .. self.MarioId, function(ply, mv, cmd)
			if self.MovementOnly and ply == LocalPlayer() and IsValid(self) then
				if g_VR and g_VR.active then return end
				if self.vrSwimming then return end
				
				-- Stop the engine from applying gravity and walking physics
				mv:SetForwardSpeed(0)
				mv:SetSideSpeed(0)
				mv:SetUpSpeed(0)
				mv:SetVelocity(Vector(0,0,0))
				
				-- Feed the smooth position directly into the prediction engine
				if self.basePos and not self.vrSwimming then
					mv:SetOrigin(self.basePos)
				end
			end
		end)
		end
		...
		local entFilter = {}
		-- CalcView is only used for third-person (non-MovementOnly) mode.
		-- In MovementOnly, lerpedPos is computed in Think and pushed to the
		-- player entity directly, so GMod's native camera works without any hook.
		local function MarioCalcView(ply, origin, angles, fov, znear, zfar)
			...
			local t = math.Clamp((SysTime() - fixedTime) / G64_TICKRATE, 0, 1)
			if stateBuffers[self.MarioId][self.bufferIndex + 1][1] ~= nil then
				if not ply:InVehicle() then
					...
					self.lerpedPos = LerpVector(t, prevState[1], curState[1]) + upOffset
					if GetConVar("g64_interpolation"):GetBool() == true then
						self.lerpedForward = LerpVector(t, g64utils.FacingToForward(prevState[3]), g64utils.FacingToForward(curState[3]))
					else
		...
		local function MarioTick()
			...
			-- Movement-only swim bypass: let GMod handle swimming
			if self.vrSwimming and self.MovementOnly then
				-- Keep ticking SM64 with zero input so state stays valid, but freeze position
				libsm64.MarioTick(self.MarioId, self.bufferIndex, lPlayer:GetAimVector(), Vector(), false, false, false)
				...
				-- Keep Mario healed
				if self.marioHealth < 8 then libsm64.MarioHeal(self.MarioId, 8) end
				-- Snap Mario to player each tick so it tracks
			...
			if self.canInput then
				...
				-- VRMod input override
				if g_VR and g_VR.active and g_VR.input and g_VR.tracking then
					-- Thumbstick movement (raw; SM64 uses facing for direction)
					local walkDir = g_VR.input.vector2_walkdirection
					if walkDir then
						inputs[1].x = walkDir.x or 0
						inputs[1].z = -(walkDir.y or 0)
						local mag = math.sqrt(inputs[1].x * inputs[1].x + inputs[1].z * inputs[1].z)
						if mag > 1 then inputs[1].x = inputs[1].x / mag; inputs[1].z = inputs[1].z / mag end
					end
					-- Jump
					if g_VR.input.boolean_jump then inputs[2] = true end
					-- Crouch: button OR physical duck below height slider
					if g_VR.input.boolean_crouch then
						inputs[4] = true
						self.vrCrouchActive = true
					elseif g_VR.tracking.hmd and g_VR.origin then
						local crouchH = GetConVar("g64_vr_crouch_height"):GetFloat()
						if g_VR.tracking.hmd.pos.z < g_VR.origin.z + crouchH then
							inputs[4] = true
							self.vrCrouchActive = true
						else
							self.vrCrouchActive = false
					...
					-- Attack: trigger and/or motion punch per mode slider
					local atkMode = GetConVar("g64_vr_attack_mode"):GetInt()
					local triggerAtk = (atkMode == 0 or atkMode == 2) and g_VR.input.boolean_primaryfire
					local motionAtk = false
					if atkMode >= 1 then
						local thresh = GetConVar("g64_vr_punch_threshold"):GetFloat()
						local rh = g_VR.tracking.pose_righthand
						local lh = g_VR.tracking.pose_lefthand
						if rh and rh.vel and rh.vel:Length() / 100 > thresh then motionAtk = true end
						if lh and lh.vel and lh.vel:Length() / 100 > thresh then motionAtk = true end
					end
					if triggerAtk or motionAtk then inputs[3] = true end
				end
				-- Block attack in movement-only non-VR
				if self.MovementOnly and not (g_VR and g_VR.active) then
					inputs[3] = false
				end
			...
			-- VR: use HMD forward for movement direction
			if g_VR and g_VR.active and g_VR.tracking and g_VR.tracking.hmd then
				facing = g_VR.tracking.hmd.ang:Forward()
			end
			if IsValid(self.cameraOverride) then
			...
			-- Movement-only: detect swimming ? hand off to GMod
			if self.MovementOnly and not self.vrSwimming then
				if bit.band(self.marioAction, 0x00002000) ~= 0 then -- ACT_FLAG_SWIMMING
					self.vrSwimming = true
				end
			end
			...
			if not self.MovementOnly then
				...
				-- In movement-only mode, keep Mario at full health always
				if self.marioHealth < 8 then
		...
		local lastKnownPlyPos = lPlayer:GetPos()
		local function VehicleTick()
			-- Detect teleport/respawn: snap Mario to player if they jumped far
			if self.MovementOnly and self.lerpedPos then
				local plyPos = lPlayer:GetPos()
				local distSqr = plyPos:DistToSqr(lastKnownPlyPos)
				if distSqr > 250000 then -- 500^2: teleport threshold
					libsm64.SetMarioPosition(self.MarioId, plyPos)
					self.marioPos = plyPos
					self.lerpedPos = plyPos
					libsm64.SetMarioAction(self.MarioId, g64types.SM64MarioAction.ACT_IDLE)
				end
				lastKnownPlyPos = plyPos
			end
			if lPlayer:InVehicle() then
				...
				if not self.InVehicle then
					...
					-- Only remove CalcView hook in third-person mode; MovementOnly doesn't use it
					if not self.MovementOnly then
						hook.Remove("CalcView", "G64_CALCVIEW" .. self.MarioId)
					end
				...
				if self.InVehicle == true then
					...
					-- Only restore CalcView hook in third-person mode
					if not self.MovementOnly then
...
function self:Think()
			...
			if self.vrSwimming and self.MovementOnly and lPlayer:OnGround() then
				local plyPos = lPlayer:GetPos()
				libsm64.SetMarioPosition(self.MarioId, plyPos)
				self.marioPos = plyPos
				self.lerpedPos = plyPos
			...
			if self.MovementOnly and not lPlayer:InVehicle() then
				if stateBuffers[self.MarioId] and stateBuffers[self.MarioId][self.bufferIndex + 1][1] ~= nil then
					-- CRITICAL: CurTime() automatically pauses when the game pauses
					local t = math.Clamp((SysTime() - fixedTime) / G64_TICKRATE, 0, 1)
					
					local curState  = stateBuffers[self.MarioId][self.bufferIndex + 1]
					local prevState = stateBuffers[self.MarioId][1 - self.bufferIndex + 1]
					
					local basePos = LerpVector(t, prevState[1], curState[1])
					self.basePos = basePos -- Saved for the SetupMove hook
					self.lerpedPos = basePos + upOffset
					
					if GetConVar("g64_interpolation"):GetBool() then
						self.lerpedForward = LerpVector(t, g64utils.FacingToForward(prevState[3]), g64utils.FacingToForward(curState[3]))
					else
						self.lerpedForward = g64utils.FacingToForward(curState[3])
					end
					...
					-- lPlayer:SetPos() is gone! SetupMove handles this now.
				end
			end
		...
		hook.Add("HUDShouldDraw", "G64_HUD_SHOULD_DRAW" .. self.MarioId, function(name)
			if self.MovementOnly then return end
			if hideHud[name] == true then return false end
		end)

		-- VR Crouch debug HUD
		hook.Add("HUDPaint", "G64_VR_DEBUG_CROUCH" .. self.MarioId, function()
			if not GetConVar("g64_vr_debug_crouch"):GetBool() then return end
			if not self.MovementOnly then return end
			if not (g_VR and g_VR.active and g_VR.tracking and g_VR.tracking.hmd and g_VR.origin) then return end

			local hmdZ = g_VR.tracking.hmd.pos.z
			local originZ = g_VR.origin.z
			local crouchH = GetConVar("g64_vr_crouch_height"):GetFloat()
			local headHeight = hmdZ - originZ
			local isCrouching = self.vrCrouchActive
			local isSwimming = self.vrSwimming

			local x, y = 20, ScrH() - 140
			local col = isCrouching and Color(255, 100, 100) or Color(100, 255, 100)

			draw.SimpleText("G64 VR Debug", "DermaDefaultBold", x, y, Color(255, 255, 255), TEXT_ALIGN_LEFT)
			y = y + 18
			draw.SimpleText("Head Height: " .. math.Round(headHeight, 1), "DermaDefault", x, y, Color(255, 255, 255), TEXT_ALIGN_LEFT)
			y = y + 16
			draw.SimpleText("Crouch Threshold: " .. math.Round(crouchH, 1), "DermaDefault", x, y, Color(255, 255, 255), TEXT_ALIGN_LEFT)
			y = y + 16
			draw.SimpleText("Crouching: " .. (isCrouching and "YES" or "NO"), "DermaDefaultBold", x, y, col, TEXT_ALIGN_LEFT)
			y = y + 16
			if isSwimming then
				draw.SimpleText("Swimming: YES (GMod control)", "DermaDefaultBold", x, y, Color(100, 150, 255), TEXT_ALIGN_LEFT)
				y = y + 16
			end

			-- Draw threshold bar
			local barX, barW, barH = x, 200, 8
			y = y + 4
			surface.SetDrawColor(60, 60, 60, 200)
			surface.DrawRect(barX, y, barW, barH)
			local fill = math.Clamp(headHeight / 80, 0, 1)
			surface.SetDrawColor(col)
			surface.DrawRect(barX, y, barW * fill, barH)
			-- Draw threshold marker
			local markerX = barX + (crouchH / 80) * barW
			surface.SetDrawColor(255, 255, 0)
			surface.DrawRect(markerX - 1, y - 2, 2, barH + 4)
		end)
		...
		-- Only register CalcView for third-person mode.
		-- MovementOnly uses the native GMod camera via lPlayer:SetPos() in Think.
		if not self.MovementOnly then
			hook.Add("CalcView", "G64_CALCVIEW" .. self.MarioId, MarioCalcView)

			-- Smooths the viewmodel by attaching it to the overridden CalcView output,
			-- while preserving internal weapon animations, sway, and bob.
			hook.Add("CalcViewModelView", "G64_CALCVIEWMODELVIEW" .. self.MarioId, function(wep, vm, oldPos, oldAng, pos, ang)
				if self.camDistance == 0 and self.view and self.view.origin then
					-- Extract the local animation offset (how much the viewmodel moved from its base)
					local animPos, animAng = WorldToLocal(pos, ang, oldPos, oldAng)
					-- Re-apply that animation offset to our custom, smoothed camera position
					local finalPos, finalAng = LocalToWorld(animPos, animAng, self.view.origin, self.view.angles)
